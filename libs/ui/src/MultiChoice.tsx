...
import { useSet, SetHookActions } from 'widgetarian-react-hooks/use-set';

export type MultiChoiceValue = string | number | undefined;

interface MultiChoiceContextValue extends SetHookActions<MultiChoiceValue> {
...
export const MultiChoice: MultiChoiceComponent = ({
  ...
  const [choiceValue, { set, ...choiceActions }] = useSet<MultiChoiceValue>(
    new Set(value)
  );
...
const MultiChoiceOption: FC<MultiChoiceOptionProps> = ({
  ...
  const Render = useMemo(
    () => render || groupRender || MultiChoiceOptionDefault,
    [render, groupRender]
  );
