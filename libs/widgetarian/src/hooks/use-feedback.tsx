...
export const useFeedback = <T extends HTMLElement = HTMLElement>({
  ...
  const handlerMouseDown = useCallback((e: MouseEvent) => setActive(true, e), [setActive]);
  const handlerMouseUp = useCallback((e: MouseEvent) => setActive(false, e), [setActive]);
  const handlerTouchStart = useCallback((e: TouchEvent) => setTouch(true, e), [setTouch]);
  const handlerTouchEnd = useCallback((e: TouchEvent) => setTouch(false, e), [setTouch]);
  const handlerMouseOver = useCallback((e: MouseEvent) => setHover(true, e), [setHover]);
  const handlerMouseOut = useCallback((e: MouseEvent) => setHover(false, e), [setHover]);
  const handlerFocus = useCallback((e: FocusEvent) => setFocus(true, e), [setFocus]);
  const handlerBlur = useCallback((e: FocusEvent) => setFocus(false, e), [setFocus]);
