import { createCss } from '../utils/css'
import { Portal } from './Portal'

import type { FC, PropsWithChildren, ReactNode } from 'react'
import type { CSSAttribute } from '../utils/css'

export const MODAL_EFFECTS = [
  'scale-up', 'slide-in', 'rotate-in', 'super-scaled',
  'sticky-up', 'sticky-bottom', 'flip-x', 'flip-y',
  'newspaper', 'fall', 'sign',
] as const
...
export type ModalRender<T> = (shown: boolean) => T
...
interface ModalProp extends PropsWithChildren {
  ...
  effect?: ModalEffect
  className?: string
  styles?: CSSAttribute | ModalRender<CSSAttribute>
  position?: ModalPosition
  overlay?: ReactNode | ModalRender<ReactNode>
}

const is3dEffect = (effect?: string) => effect && [
  'fall',
  'sign',
  'flip-x',
  'flip-y',
...
const getPos = (position: ModalPosition = 'center') => {
  const normalized = position === 'top' ? 'top-center'
...
const modalContentStyle = ({ shown, effect, styles = {} }: ModalProp) => createCss({
  position: 'relative',
  opacity: shown ? 1 : 0,
  width: '50%',
  maxWidth: '630px',
  ...
  ...(effect === 'fall' && {
	  transform: `translateZ(${shown ? '0px' : '600px'}) rotateX(${shown ? '0deg' : '60deg'})`,
  }),
  ...(effect === 'rotate-in' && {
	  transform: `translateY(${shown ? '0%' : '100%'}) rotateX(${shown ? '0deg' : '90deg'})`,
  }),
  ...(effect === 'sign' && {
...
export const Modal: ModalComponent = ({
  ...
  const modalContentClass = [
    modalContentStyle({ shown, effect }),
    className
  ].join(' ').trim()
  return (
    <Portal id={portal}>
